<%--
    Class Name : modList.jsp
    Description : 모바일 수출정정취하신고 조회
    ...
    <script>
        ...
        $(function() {
            ...
            var params = {
                ...
                "USER_DIV"  : "${session.getUserDiv()}",
                "TG_NO"     : "${session.getTgNo()}",
                "APPLICATION_ID"  : "${session.getApplicantId()}"
            };

            var headers = [
                {"HEAD_TEXT": "수출신고번호" 	, "FIELD_NAME": "RPT_NO"	, "LINK":"fn_detail"},
                {"HEAD_TEXT": "차수"     	, "FIELD_NAME": "MODI_SEQ"},
                {"HEAD_TEXT": "수신상태"     	, "FIELD_NAME": "RECE_NM"},
                {"HEAD_TEXT": "전송상태"     	, "FIELD_NAME": "SEND_NM"},
                {"HEAD_TEXT": "신청구분" 		, "FIELD_NAME": "SEND_DIVI_NM"},
                {"HEAD_TEXT": "정정신청일자" 	, "FIELD_NAME": "MODI_DAY"}
            ];
            ...
            $('#btnSearchPopup').on('click', function () {
                layerPop("<c:url value='/jspView.do?jsp='/>" + "mob/exp/modListPopup");
            });
        ...
        function fn_detail(index){
            ...
            mfn_forward("mob/exp/modDetail", data);
        }
