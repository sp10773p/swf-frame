...
<head>
    ...
    <script>
        ...
        $(function (){
            ...
            $('#btnList').on("click", function (e) { $.comm.pageBack(); });	//목록btn
         	
            $('#btnMod').on("click", function (e) { fn_mod(); }); 			//정정btn
         	
            $('#btnCancel').on("click", function (e) { fn_cancel(); });		//취하btn
           
            $('#btnTerm').on("click", function (e) { fn_term(); }); 		//기간연장btn
       ...
       /*  	$.comm.bindCombos.addComboInfo("MOD_CD" 				, "CUS0029", true,  null, 3);	//정정사유코드
        	$.comm.bindCombos.addComboInfo("MOD_OBL_REASONCD" 		, "CUS0028", true,  null, 3);	//귀책사유코드
        	$.comm.bindCombos.addComboInfo("AMENDTYPECD" 			, "CUS0032", true,  null, 3);	//취하사유코드(수출취하사유)
        	$.comm.bindCombos.addComboInfo("OBLIGATIONREASONCD" 	, "CUS0028", true,  null, 3);	//귀책사유코드
        	$.comm.bindCombos.addComboInfo("TERM_CD" 				, "CUS0029", true,  null, 3);	//정정사유코드
        	$.comm.bindCombos.addComboInfo("TERM_OBL_REASONCD" 		, "CUS0028", true,  null, 3);	//귀책사유코드 */
        ...
        function fn_commSave(gbn){
            ...
            $.comm.sendForm("/dec/saveExpDecAll.do", "detailForm", fn_callback, "수출신고 수정", null, null, etcParams);
        }
      	
     	//정정
      	function fn_mod(){
      		if($.comm.isNull($('#EXP_LIS_DAY').val())){            		
            	alert($.comm.getMessage("W00000042")); //수리된 신고서만 정정이 가능합니다.
            	return;
        	}
        	
      		var spec = "width:800px;height:600px;scroll:auto;status:no;center:yes;resizable:yes;";
            // 모달 호출
            $.comm.dialog("<c:out value='/jspView.do?jsp=exp/dec/decDetailModPopup' />", spec,
                function () { // 리턴받을 callback 함수
                    var ret = $.comm.getModalReturnVal();
                    if (ret) {
                    	var params = [];
                  		params["RPT_NO"] = ret.RPT_NO;
                  		params["MODI_SEQ"] = ret.MODI_SEQ;
                  		params["SEND_DIVI"] = ret.SEND_DIVI;
                  		params["CALL_TYPE"] = "MOD_POP";
                    	$.comm.forward("exp/dec/modDetail", params);
                    }
                }
            );
      	}
      	...
      	function fn_cancel(){
      		if($.comm.isNull($('#EXP_LIS_DAY').val())){            		
            	alert($.comm.getMessage("W00000044")); //수리된 신고서만 취하가 가능합니다.
            	return;
        	}
      		
      		var spec = "width:800px;height:600px;scroll:auto;status:no;center:yes;resizable:yes;";
            // 모달 호출
            $.comm.dialog("<c:out value='/jspView.do?jsp=exp/dec/decDetailCancelPopup' />", spec,
                function () { // 리턴받을 callback 함수
                    var ret = $.comm.getModalReturnVal();
                    if (ret) {
     	           		  $.comm.forward("exp/dec/modList", {"RPT_NO":ret.RPT_NO});
                    }
                }
            );
      	...
      	//기간연장
      	function fn_term(){
      		$.comm.setModalArguments({"RPT_NO" : $("#RPT_NO").val(), "JUK_DAY" : $("#JUK_DAY").val()});
      		if($.comm.isNull($('#EXP_LIS_DAY').val())){            		
            	alert($.comm.getMessage("W00000043")); //수리된 신고서만 기간연장이 가능합니다.
            	return;
        	}
        	
      		var spec = "width:800px;height:600px;scroll:auto;status:no;center:yes;resizable:yes;";
            // 모달 호출
            $.comm.dialog("<c:out value='/jspView.do?jsp=exp/dec/decDetailTermPopup' />", spec,
                function () { // 리턴받을 callback 함수
	            	var ret = $.comm.getModalReturnVal();
	                if (ret) {
	 	           		  $.comm.forward("exp/dec/modList", {"RPT_NO":ret.RPT_NO});
      	...
      	function fn_validate(gbn){
      		if(gbn == "A"){
      			...
      			if(sCod=="CIF"||sCod=="CIP"||sCod=="DAF"||sCod=="DDP"||sCod=="DDU"||sCod=="DEQ"||sCod=="DES"||sCod=="DAT"||sCod=="DAP"){
      				...
      				if($.comm.isNull(sInsu)||sInsu <= 0 || (!$.comm.isNull(sFre) && sFre > 0 )){
      					alert("인도조건이 "+ sCod+"이면 보험료는 0보다 커야하고, 운임은 0이어야 합니다..");
      					return false;
      				}
      			}else if(sCod=="CFR"||sCod=="CPT"){
      				if((!$.comm.isNull(sInsu) && sInsu > 0) || $.comm.isNull(sFre) || sFre <= 0){
      					alert("인도조건이 "+ sCod+"이면 보험료는 0이어야 하고, 운임은 0보다 커야 합니다..");
      					return false;
      				}
      			}else if(sCod=="FOB"||sCod=="EXW"||sCod=="FAS"||sCod=="FCA"){
      				if((!$.comm.isNull(sInsu) && sInsu > 0) || (!$.comm.isNull(sFre) && sFre > 0)){
      					alert("인도조건이 "+ sCod+"인 경우에는 운임,보험료 둘다 0이어야 합니다..");
      					return false;
      				}
      			}
      	...
      	function fn_zipPopup(){
      		var spec = "width:900px;height:800px;scroll:auto;status:no;center:yes;resizable:yes;";
            // 모달 호출
...
<div class="inner-box">
    <form id="detailForm" name="detailForm" method="post">
    	...
    	<input type="hidden" id="COMM_CODE" name="COMM_CODE" />
    	<input type="hidden" id="EXP_CODE" name="EXP_CODE" />
    	<input type="hidden" id="RPT_MARK" name="RPT_MARK" />
				...
				<div class="title_btn_frame clearfix" style="padding-bottom: 10px;">
					<div class="title_btn_inner">
						<a href="#" class="title_frame_btn" id="btnList">목록</a>
		               	...
		               	<c:if test="${RECE ne NULL && RECE ne '' && RECE ne '01' && RECE ne '90'}">
		               		<a href="#" class="title_frame_btn" id="btnTerm">기간연장</a>
		               		<a href="#" class="title_frame_btn" id="btnMod">정정</a>
		               		<a href="#" class="title_frame_btn" id="btnCancel">취하</a>
		               	</c:if>
			...
			<div class="title_frame" style="margin-bottom: 5px;">
				...
				<div class="vertical_frame">
	               	<div class=vertical_frame_left37>
	               		<div id="gridRanLayer" style="height: 243px;"></div>
	               	</div>
               		<div class="vertical_frame_right37">
						<div class="table_typeA gray table_toggle">
	                        ...
	                        <tr>
	                            ...
	                            <td colspan="5"><input type="text" name="MODEL_GNM" id="MODEL_GNM" <attr:length value="30" /> <attr:alphaNumber/> ></td>
	                        </tr>
	                        <tr>
	                            <td>
